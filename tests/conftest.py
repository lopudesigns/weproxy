...
@pytest.fixture
def node_response_schema():
    with open(os.path.join(SCHEMA_DIR, 'node-response-schema.json')) as f:
        return ujson.load(f)
...
def node_requests_and_responses():
    with open(os.path.join(REQS_AND_RESPS_DIR, 'node.json')) as f:
        return ujson.load(f)


def batched_node_requests_and_responses(chunk_size=15):
    requests = chunks(
        [req for req, resp in node_requests_and_responses()], chunk_size)
    responses = chunks(
        [resp for req, resp in node_requests_and_responses()], chunk_size)
...
def combined_requests_and_responses():
    return node_requests_and_responses() + appbase_requests_and_responses()
...
@pytest.fixture
def translatable_node_requests_and_responses():
    import jussi.urn
    untranslateable = frozenset(['get_liquidity_queue', 'get_miner_queue',
                                 'get_discussions_by_payout'])
    return [(req, resp) for req, resp in node_requests_and_responses()
            if jussi.urn.from_request(req).method not in untranslateable]


def batch_translatable_requests_and_responses(chunk_size=15):
    requests = chunks(
        [req for req, resp in translatable_node_requests_and_responses()], chunk_size)
    responses = chunks(
        [resp for req, resp in translatable_node_requests_and_responses()], chunk_size)
    return list(zip(requests, responses))
...
NODE_JSON_RPC_CALLS = [
    ...
    },
    {
        ...
        'method': 'call',
        'params': ['database_api',
                   'get_conversion_requests', ['WeYouMe']]
    },
    {
        'id': 12,
        'jsonrpc': '2.0',
...
NODE_JSONRPC_CALL_PAIRS = []
for c in NODE_JSON_RPC_CALLS:
    if c['method'] == 'call':
        method = c['params'][1]
        new_method = [
            m for m in NODE_JSON_RPC_CALLS if m['method'] == method]
        NODE_JSONRPC_CALL_PAIRS.append((c, new_method[0]))
...
URN_TEST_REQUEST_DICTS = [
    ...
    # node, bare method, empty params list
    ({
        'id': 5021,
        'jsonrpc': '2.0',
        'method': 'get_dynamic_global_properties',
        ...
        'node.database_api.get_dynamic_global_properties.params=[]',
        'wss://api.WeYouMe.io',
        1,
        3
    ),
    # node, bare method, params list
    ({
        'id': 5022,
        'jsonrpc': '2.0',
        'method': 'get_block',
        ...
        'node.database_api.get_block.params=[1]',
        'wss://api.WeYouMe.io',
        -2,
        3
    ),

    # node, bare_method, account transfer url
    ({
        "id": 5023,
        "jsonrpc": "2.0",
        "method": "get_state",
        ...
        'node.database_api.get_state.params=["\/@justinw\/transfers"]',
    ...
    # node, method=call, empty params list
    ({
        'id': 5024,
        'jsonrpc': '2.0',
        'method': 'call',
    ...
    ({
        ...
        'node.database_api.find_accounts.params=[]',
        'wss://api.WeYouMe.io',
        3,
        3
    ),
    # node, method=call, account transfer url
    ({
        "id": 5027,
        "jsonrpc": "2.0",
        "method": "call",
                 ...
                 'operations': [
                     [
                         'customJson',
                         {
                             "required_auths": [],
                             "id": "follow",
                             "json": "{\"follower\":\"WeYouMe\",\"following\":\"WeYouMe\",\"what\":[\"posts\"]}",
                 ...
                 'operations': [
                     [
                         'customJson',
                         {
                             ...
                             "id": "follow",
                             "json": "{\"follower\":\"WeYouMe\",\"following\":\"WeYouMe\",\"what\":[\"posts\"]}",
                             "required_posting_auths": ["non-WeYouMe"]
                         }
                     ]
                 ]
                 }
...
@pytest.fixture
def node_jrpc_response_validator(node_response_schema):
    return rpartial(jsonschema.validate, node_response_schema)
...
@pytest.fixture(params=NODE_JSONRPC_CALL_PAIRS)
def node_method_pairs(request):
    yield request.param


@pytest.fixture(
    scope='function', params=node_requests_and_responses(),
    ids=lambda reqresp: str(URN(*reqresp[0])))
def node_request_and_response(request):
    yield copy.deepcopy(request.param[0]), copy.deepcopy(request.param[1])
...
@pytest.fixture(params=translatable_node_requests_and_responses())
def translatable_node_request_and_response(request):
    yield copy.deepcopy(request.param)
...
@pytest.fixture
def node_jussi_request_and_dict(node_request_and_response):
    jsonrpc_request, _ = node_request_and_response
    dummy_request = make_request()
